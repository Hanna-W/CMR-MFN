...
class FOSTERNet(nn.Module):
    ...
    def forward(self, x):
        ...
        out["logits"] = out["logits"].view((-1, 8) + out["logits"].size()[1:])
        out["logits"] = self.consensus(out["logits"])
        ...
        out["fe_logits"] = out["fe_logits"].view((-1, 8) + out["fe_logits"].size()[1:])
        out["fe_logits"] = self.consensus(out["fe_logits"])
        out["fe_logits"] = out["fe_logits"].squeeze(1)

        if self.oldfc is not None:
            old_logits = self.oldfc(features[:, : -self.out_dim])["logits"]
            out.update({"old_logits": old_logits})
            out["old_logits"] = out["old_logits"].view((-1, 8) + out["old_logits"].size()[1:])
            out["old_logits"] = self.consensus(out["old_logits"])
            out["old_logits"] = out["old_logits"].squeeze(1)
    ...
    def update_fc(self, nb_classes):
        ...
        if self.out_dim is None:
####################################################################################################
            self.out_dim = 1024*4#self.convnets[-1].out_dim
        fc = self.generate_fc(self.feature_dim, nb_classes)
        if self.fc is not None:
            nb_output = self.fc.out_features
            weight = copy.deepcopy(self.fc.weight.data)
