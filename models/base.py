...
class BaseLearner(object):
    ...
    def _evaluate(self, y_pred, y_true):
        ret = {}
        grouped = accuracy(y_pred.T[0], y_true, self._known_classes, self._total_classes - self._known_classes)
        ret["grouped"] = grouped
    ...
    """
    Add one function by myself
    """

    def save_scores(self, results, labels, predicts, scores_file):
        save_dict={}
        keys = ["features", "fusion_features", "logits"]
        save_dict = {k : np.array([result[k] for result in results]) for k in keys}
